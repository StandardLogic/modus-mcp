 ...
 * intentionally excluded — they are not emitted by the verify pipeline. If a
 * future taxonomy needs them, introduce `PASSPORT_REGISTER_REASON_CODES`
 * separately.
 *
 * Canonicalization reason codes are not in this set either — see
 * `CanonicalizationError.reasonCode`. Introduce `CANONICALIZATION_REASON_CODES`
 * if a second value ever appears.
 */

export type PassportVerifyReasonCode =
...
export const PASSPORT_VERIFY_REASON_CODES: ReadonlySet<PassportVerifyReasonCode> = new Set<PassportVerifyReasonCode>([
  'invalid_envelope_shape',
  'unsupported_schema_version',
  'signature_malformed',
  'signature_invalid',
